...
from ..config import USER_AGENT, MAX_JS_BROWSERS
from .utils import normalize_url

logger = logging.getLogger("webxguard.fetcher")
...
_GENERIC_BOT = [
    ...
    "prove you are human",
    "anti-ddos",
    "press & hold",                     # Kasada challenge
    "just a moment",                    # CF interstitial text
]

# All signals combined at module level (avoids re-concat on every call)
...
def detect_antibot(
    ...
    Checks (in order):
      ...
      4. JS-only redirect challenge pages (very short page, JS redirect, no body)
    ...
    try:
        ...
        if headers:
            ...
            # Server header hints — only flag if page looks suspicious too
            server = h.get("server", "")
        ...
        # Typical for old Cloudflare IUAM: very short, JS redirect, no <body>
...
domain_network_files: Dict[str, str]     = {}
domain_screenshot_files: Dict[str, str]  = {}
...
def _incremented_path(base_dir: str, domain: str, ext: str) -> str:
    base = os.path.join(base_dir, f"{domain}.{ext}")
    if not os.path.exists(base):
        return base
    i = 1
    while True:
        new_path = os.path.join(base_dir, f"{domain}_{i}.{ext}")
        if not os.path.exists(new_path):
            return new_path
        i += 1


def _safe_write_zip(zip_path: str, filename: str, data: bytes):
    try:
        with zipfile.ZipFile(zip_path, "a", zipfile.ZIP_DEFLATED) as zf:
            zf.writestr(filename, data)
    except Exception as e:
        logger.error(f"ZIP write failed: {e}")


def _batch_log(path: str, page_id: int, events: list):
    try:
...
def get_network_log_file(domain: str) -> str:
    """Return the JSONL log path for a domain (same path within a single run)."""
    if domain not in domain_network_files:
        domain_network_files[domain] = _incremented_path(NETWORK_LOG_DIR, domain, "jsonl")
    return domain_network_files[domain]
...
class JSBrowserPool:
    ...
    async def start(self):
        try:
            ...
            logger.exception(f"Browser start failed: {e}")
    ...
    async def fetch_js(
        ...
        """
        Fetch a URL using a real browser and collect everything needed
        for passive scanning.

        Returns:
            (html, dynamic_links, js_endpoints, ws_endpoints,
             network_log_path, screenshot_zip_path)

        Every HTTP network event is shaped for scanner modules:
            {
                type, url, method, status_code, headers, cookies,
                set_cookies, body, security_headers, request_headers,
                request_body, resource_type, timestamp
            }

        _batch_log is called ONLY at the very end after cookies are injected
        into all HTTP events — guarantees the log always has cookies populated.
        """
        if not self.context:
        ...
        if domain not in domain_screenshot_files:
            domain_screenshot_files[domain] = _incremented_path(
                SCREENSHOTS_DIR, domain, "zip"
            )
        screenshot_zip = domain_screenshot_files[domain]

        if domain not in domain_network_files:
            domain_network_files[domain] = _incremented_path(
                NETWORK_LOG_DIR, domain, "jsonl"
            )
        network_log = domain_network_files[domain]
        ...
        async with self.semaphore:
            for attempt in range(1, max_retries + 1):
                ...
                try:
                    ...
                    # Snapshot cookies already in jar BEFORE this page load
                    cookies_before = {
                        c["name"]: c
                    ...
                    # ── RESPONSE HANDLER ──────────────────────────────────────
                    # cookies field is intentionally left empty here —
                    # it is injected in post-processing AFTER the full jar
                    # is available (see STEP 2 below).
                    async def handle_response(resp):
                        try:
                            key      = f"{resp.request.method}::{resp.request.url}"
                            req_info = request_map.pop(key, None)

                            # headers_array() preserves duplicate header names —
                            # critical for multiple Set-Cookie headers
                            try:
                                headers_array = await resp.headers_array()
                            except Exception as e:
                                logger.debug(
                                    f"[Fetcher] headers_array() failed, "
                                    f"falling back: {e}"
                                )
                                try:
                            ...
                            scannable    = any(t in content_type for t in (
                                "text/",
                                "application/json",
                            ...
                            if scannable:
                                try:
                                    ...
                                    logger.debug(
                                        f"[Fetcher] Body read failed for "
                                        f"{resp.request.url}: {e}"
                                    )
                            ...
                            logger.debug(
                                f"[Fetcher] handle_response error on "
                                f"{getattr(resp.request, 'url', '?')}: {e}"
                            )
                    ...
                    # ── NAVIGATE ──────────────────────────────────────────────
                    goto_timeout_sec = goto_timeout / 1000 + 5
                    await asyncio.wait_for(
                        ...
                        timeout=goto_timeout_sec,
                    )

                    # ── ANTI-BOT CHECK (with full header context) ─────────────
                    initial_html = await page.content()

                    # Find headers & status from the main document network event
                    ...
                    if detect_antibot(initial_html, main_status, main_headers):
                        logger.warning(
                            f"[Fetcher] Anti-bot detected on {url} "
                    ...
                    if wait_for_selector:
                        try:
                            await page.wait_for_selector(
                                wait_for_selector, timeout=js_wait
                            )
                        except Exception:
                            pass
                    else:
                    ...
                    try:
                        ss = await asyncio.wait_for(
                            ...
                            timeout=screenshot_timeout / 1000 + 5,
                        )
                        filename = (
                            f"{int(datetime.now(timezone.utc).timestamp() * 1000)}.png"
                        )
                        await asyncio.to_thread(
                            _safe_write_zip, screenshot_zip, filename, ss
                        )
                        ss_path = screenshot_zip
                    except Exception:
                        logger.debug("[Fetcher] Screenshot failed.")

                    # ══════════════════════════════════════════════════════════
                    # POST-PROCESSING — runs after full page load, before disk
                    # ══════════════════════════════════════════════════════════
                    ...
                    # STEP 5b: Final fallback — from domain cookies in jar
                    if not all_set_cookies and parsed_cookies:
                        ...
                        if domain_cookies:
                            logger.debug(
                                f"[Fetcher] Reconstructing {len(domain_cookies)} "
                                f"from domain jar for {url}"
                            )
                            all_set_cookies = [
                    ...
                    logger.info(
                        f"[Fetcher] Cookie summary for {url}: "
                        f"{len(parsed_cookies)} in jar, "
                        f"{len(newly_set)} newly set, "
                        f"{len(all_set_cookies)} Set-Cookie strings available"
                    )
                    ...
                    # STEP 8: Write to disk LAST — after cookies injected
                    if network_events:
                        await asyncio.to_thread(
                    ...
                    return html, dynamic_links, js_eps, ws_eps, network_log, ss_path

                except Exception as e:
                    logger.warning(
                        f"[Fetcher] Attempt {attempt} failed for {url}: {e}"
                    )
